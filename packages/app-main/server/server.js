	...
	updateFriends: function() {
		...
		if (Meteor.user().services.facebook.accessToken) {
			...
			graph.get(str, Meteor.bindEnvironment(function(err, result) {
				...
				console.log("couldn't wrap callback");
			}));
		}
	},
	fbUpdate: function(s) {
	...
	addRef: function(uId) {
		...
		if (Meteor.user().new) {
			if (user.profile.referrals) {
				Meteor.users.update({_id: uId}, {$push: {'profile.referrals': Meteor.userId()}});
			} else {
				Meteor.users.update({_id: uId}, {$set: {'profile.referrals': [Meteor.userId()]}});
			}
			Meteor.users.update({_id: Meteor.userId()}, {$set: {'profile.referredBy': uId}});
		}
	},
	fbUpdate: function(s) {
		Meteor.users.update({_id: this._id}, {$set: {"profile.blah": s}});
...
var admins = Meteor.users.find({
	'services.facebook.id': {
		$in: ['763201810367366', '10203937562217210', '10201231262863208'] // Steve, Keshara, Jasper
	}
}).fetch();
Debug.log('admins', admins);
