	...
	waitOn: function() {
		...
		if (Meteor.isClient) {
			...
			if (Session.get('buyOn') && !Session.get('sellOn')) {
				...
				filter = {$or: [{buy: {$exists: false}}, {buy: true}]};
			} else {
				filter = {};
			}
	...
	this.route('feeds', {
		...
		waitOn: function() {
			...
			if (Session.get('buyOn') && !Session.get('sellOn')) {
				...
				filter = {_id: {$in: Feeds.findOne({_id: this.params.id}).items}, $or: [{buy: {$exists: false}}, {buy: true}]};
			} else {
				filter = {_id: {$in: Feeds.findOne({_id: this.params.id}).items}};
			}
			return Meteor.subscribe('items', filter, {sort: {score: -1}, limit: Session.get('ftoiLimit')});
		},
		data: function() {
