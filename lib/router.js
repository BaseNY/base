	...
	waitOn: function() {
		...
		if (Meteor.isClient) {
			...
			if(Session.get('degrees')){
				if(Session.get('degrees') == 1) {
					if (Session.get('sellOn')) {
		                                filter = {$or: [{buy: {$exists: false}}, {buy: {$exists: Session.get('buyOn')}}], fbId: {$in: Meteor.user().profile.friends}}
	...
	this.route('testmessagehome', {
		...
		action: function() {
			if (this.ready()) {
				...
				this.redirect('/testmessage/' + conversation._id);
			}
		}
	});

	this.route('testmessage', {
		path: '/testmessage/:conversationId',
