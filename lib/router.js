(function() {
	...
	HomeController = FastRender.RouteController.extend({
		...
		waitOn: function() {
			...
			if (Meteor.isClient) {
        //                    return Meteor.subscribe('ftoItems', Session.get('ftoiLimit'));
...
Router.map(function() {
	...
	this.route('profile', {
		path: '/profile/:id',
		waitOn: function() {
                            return Meteor.subscribe('items', {sellerId: this.params.id}, Session.get('ftoiLimit'));
		},
		data: function() {
			return {
	...
	this.route('itemResults', {
		waitOn: function() {
			return Meteor.subscribe('items', {}, Session.get('ftoiLimit'));
		},
		fastRender: true
	});
	...
	this.route('feeds', {
		...
		template: 'home',
		waitOn: function() {
                            return Meteor.subscribe('items', {_id: {$in: Feed.findOne({_id: this.params.id}).items}}, Session.get('ftoiLimit'));
		},
		data: function() {
