if (Meteor.isClient) {
	var defaultItems = function() {
		Meteor.call('createItem', {name: 'Shoe', desc: 'cool'}, function(err, res) {
		});
		Meteor.call('createItem', {name: 'iPhone', desc: 'apple'}, function(err, res) {
		});
		Meteor.call('createItem', {name: 'elastic search', desc: 'more than two apples'}, function(err, res) {
		});
		Meteor.call('createItem', {name: 'Dog', desc: 'cute and knows tricksies'}, function(err, res) {
		});
	};
	//defaultItems();
